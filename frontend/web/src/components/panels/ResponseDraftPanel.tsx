...
export default function ResponseDraftPanel({ caseId, arguments_ }: { caseId: string; arguments_: Argument[] }) {
  ...
  const [body, setBody] = useState(() => buildDraft(arguments_));
  ...
  async function saveDraft() {
    ...
    try {
      const res = await fetch(`/api/v1/cases/${encodeURIComponent(caseId)}/response-drafts`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, recipient_name: recipient || undefined, subject, body }),
      });
  ...
  return (
    <div className="space-y-3 rounded-xl surface-flat p-4 border border-fp-border" role="region" aria-label="Response Draft">
      <div className="flex items-start justify-between gap-3">
        <div><h3 className="text-sm font-semibold text-fp-text flex items-center gap-2"><FileSignature className="w-4 h-4 text-fp-blue" />Response Draft</h3><p className="text-xs text-fp-text-muted mt-0.5">Human-reviewable work product. Nothing is mailed until explicitly authorized.</p></div>
        <button onClick={() => window.print()} className="p-2 rounded-lg bg-fp-surface-2 border border-fp-border text-fp-text-muted hover:text-fp-text" title="Print or save as PDF" aria-label="Print or save as PDF"><Printer className="w-4 h-4" /></button>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        <label className="text-xs text-fp-text-muted">Document title<input value={title} onChange={e => setTitle(e.target.value)} className="mt-1 w-full rounded-lg bg-fp-surface-2 border border-fp-border px-3 py-2 text-sm text-fp-text" /></label>
        <label className="text-xs text-fp-text-muted">Recipient<input value={recipient} onChange={e => setRecipient(e.target.value)} placeholder="Agency / hearing officer" className="mt-1 w-full rounded-lg bg-fp-surface-2 border border-fp-border px-3 py-2 text-sm text-fp-text" /></label>
      </div>
      ...
      {saved && <div className="text-xs text-fp-green bg-fp-green/10 border border-fp-green/30 rounded-lg px-3 py-2">Draft saved to this case.</div>}
