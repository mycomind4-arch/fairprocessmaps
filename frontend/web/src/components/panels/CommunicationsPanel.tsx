...
import { CheckCircle2, FileText, Loader2, Mail, Plus, RefreshCw, Truck, XCircle } from "lucide-react";
...
export default function CommunicationsPanel({ caseId }: { caseId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/v1/cases/${caseId}/communications`, { cache: "no-store" });
      if (!response.ok) throw new Error(`Could not load communications (${response.status})`);
  ...
  async function createDraft(event: FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm((current) => ({ ...current, recipient_name: "", recipient_company: "", recipient_address1: "", recipient_address2: "", recipient_city: "", recipient_postal_code: "" }));
      await load();
    } catch (err) {
    ...
    <section className="space-y-4" aria-label="Communications and mailing">
      <div className="glass rounded-xl p-4 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-fp-text-muted mt-1">Prepare formal responses, track physical mailing, and keep proof attached to the case.</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={createDraft} className="surface-flat rounded-xl p-4 border border-fp-border space-y-3">
          ...
          <p className="text-xs text-fp-text-muted">This creates a case-linked mailing draft. Physical fulfillment remains a separate MailMyPDF execution step.</p>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button type="submit" disabled={creating} className="px-3 py-2 rounded-lg bg-fp-blue text-white text-sm font-medium flex items-center gap-2 disabled:opacity-50">{creating && <Loader2 className="w-4 h-4 animate-spin" />} Create Draft</button>
          </div>
        </form>
      )}
