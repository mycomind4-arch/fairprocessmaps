...
const MAP_STYLE: StyleSpecification = {
  ...
  layers: [
    ...
    { id: "parcels", type: "raster", source: "parcels", paint: { "raster-opacity": 0.82 } },
  ],
};
...
export function LandingMap() {
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>(HUMBOLDT_CENTER);
  ...
  function locateUser() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setMapCenter(point);
        mapRef.current?.flyTo({ center: point, zoom: 16, duration: 1200, essential: true });
        void identifyParcel(point[0], point[1]);
        setLocating(false);
      },
      (error) => {
        setLocating(false);
  ...
  useEffect(() => {
    ...
    map.on("click", (event) => void identifyParcel(event.lngLat.lng, event.lngLat.lat));
  ...
  return (
    <div className="relative h-[360px] w-full overflow-hidden md:h-[440px]">
      ...
      {loading && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-forest/75 backdrop-blur-sm">
          <div className="flex items-center gap-3 rounded-md bg-card/95 px-4 py-3 text-[13px] text-foreground shadow-lg">
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-forest/25 border-t-forest" /> Loading live map…
          </div>
        </div>
      )}

      <a href="/map" className="absolute left-4 right-16 top-4 z-10 flex items-center gap-2.5 rounded-md bg-card/95 px-4 py-3 shadow-md backdrop-blur-sm md:right-[36%]">
        <Search className="h-4 w-4 text-muted-foreground" />
        <span className="min-w-0 flex-1 truncate text-[13px] text-muted-foreground">Search by address, APN, or case number...</span>
      ...
      <button
        ...
        onClick={locateUser}
        disabled={locating}
        className="absolute right-3 top-[112px] z-10 flex h-9 w-9 items-center justify-center rounded-md bg-card/95 text-forest shadow-md backdrop-blur-sm transition hover:bg-card disabled:opacity-60"
      ...
      {locationError && (
        <div className="absolute left-4 right-4 top-[68px] z-20 rounded-md border border-amber-200 bg-amber-50/95 px-3 py-2 text-xs text-amber-900 shadow-md md:left-auto md:right-16 md:max-w-sm">
          {locationError}
        </div>
      )}
      ...
      <div className="absolute bottom-4 right-4 z-10 hidden w-[250px] rounded-md bg-card/95 p-3 shadow-xl backdrop-blur-sm md:block">
        ...
        <div className="mt-3 flex items-start gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[12px] text-muted-foreground">{parcel ? `${parcel.city || "Humboldt County"}, CA` : "Click anywhere on the live map"}</p>
          </div>
        </div>
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-10 rounded-md bg-forest/85 px-3 py-2 text-[11px] font-medium text-forest-foreground shadow-md backdrop-blur-sm">
        Click a parcel or use <span className="font-semibold">◎</span> to locate yourself
      </div>
    </div>
  );
}
