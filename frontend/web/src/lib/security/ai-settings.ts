...
export async function resolveEffectiveClaudeEnv<T extends object>(
  ...
  if (settings.encrypted_key) {
    try {
      apiKey = await decryptApiKey(env as AiSettingsCryptoEnv, settings.encrypted_key);
    } catch {
  ...
  return {
    ...env,
    ANTHROPIC_API_KEY: apiKey ?? (env as ClaudeBindingEnv).ANTHROPIC_API_KEY,
    ANTHROPIC_MODEL: settings.model ?? (env as ClaudeBindingEnv).ANTHROPIC_MODEL,
  };
}
