...
export function logRecordsResponseStage() {
  return async (ctx: StageContext): Promise<StageResult> => {
    ...
    if (responded) {
      ...
      if (!isCalendarDate(responseDate)) return needsInput("log_response", "A valid response date is required.", "Enter the date on the response.");
      return ok(
        "log_response",
...
// This workflow records a human's external send; it does not call a mail API.
export function confirmRecordsRequestSentStage() {
  return async (ctx: StageContext): Promise<StageResult> => {
    if (!isCalendarDate(ctx.input?.sentDate)) return needsInput("send", "Send the reviewed request using your chosen method, then record its actual send date.", "Enter the date and method after sending it yourself.");
    return ok("send", "The user confirmed sending the reviewed request outside the app.", { sentDate: ctx.input.sentDate, method: ctx.input.method ?? "unspecified" });
  };
}
