...
export default function InvestigationView() {
  ...
  const [activeDetailTab, setActiveDetailTab] = useState<"evidence" | "findings" | "authority">("evidence");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (graphRes.ok) {
        ...
        // Initialize visible node types — all on by default
      ...
      // Silently fail — UI handles null states
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  // ── Timeline event click → highlight connected graph nodes ──
  const handleEventClick = (entry: TimelineEntry) => {
    ...
    // Also highlight the case node itself
    if (graph) nodes.add(graph.case.id);
    setHighlightedNodes(nodes);
  };

  // ── Graph node click → show details ──
  ...
  // Filtered graph nodes/edges based on visible node types
    ...
    <div className="h-screen flex flex-col bg-fp-bg overflow-hidden">
      ...
      <header className="shrink-0 border-b border-fp-border bg-fp-surface/60 backdrop-blur-xl">
        <div className="flex items-center gap-4 px-6 py-3">
          <button
            ...
            className="text-fp-text-dim hover:text-fp-text transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-fp-text-dim mt-0.5">
                ...
                <span>•</span>
                <span className={
                  summary.status === "open" ? "text-fp-amber" : "text-fp-green"
                }>{summary.status}</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-6 px-6 py-2 border-t border-fp-border/50 text-xs text-fp-text-dim">
          ...
          <span className="flex items-center gap-1.5">
            <AlertTriangle className="w-3 h-3" />
            {summary.open_findings_count} open findings
            {summary.critical_findings_count > 0 && (
              <span className="text-fp-red">({summary.critical_findings_count} critical)</span>
            )}
          </span>
          <span className="flex items-center gap-1.5">
            <Clock className="w-3 h-3" />
            {summary.timeline_event_count} events
          </span>
          {summary.last_action.date && (
            <span className="flex items-center gap-1.5">
              Last: {summary.last_action.type_label || summary.last_action.type}
              <span className="text-fp-text-muted">{summary.last_action.date}</span>
            </span>
          )}
        </div>
      </header>

      {/* ── Main content: Timeline | Graph ── */}
                ...
                return (
                  <button
                    ...
                    className={`px-1.5 py-0.5 rounded text-[10px] font-medium transition-colors ${
                      active
                        ? "bg-fp-blue/20 text-fp-blue border border-fp-blue/30"
      ...
      <div className="h-64 shrink-0 border-t border-fp-border bg-fp-surface/60 backdrop-blur-xl overflow-hidden flex flex-col">
        <DetailPanel
          ...
          summary={summary}
          selectedNode={selectedNode}
          selectedEvent={timeline?.events.find((e) => e.id === selectedEvent) ?? null}
