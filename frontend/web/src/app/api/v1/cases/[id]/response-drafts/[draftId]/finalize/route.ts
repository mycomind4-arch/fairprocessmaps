...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; draftId: string }> }) {
  try {
    ...
    const existing = await env.DB.prepare(`SELECT id, title, status, r2_key, mime_type, file_size FROM evidence WHERE response_draft_id = ? AND case_id = ? AND organization_id = ? LIMIT 1`)
      .bind(draftId, caseId, auth.user.organization_id).first<Record<string, unknown>>();
    if (existing && draft.status === "finalized") {
      return NextResponse.json({ ok: true, data: { ...draft, evidence: existing }, error: null });
    }

    const now = new Date().toISOString();
    const pdf = renderResponsePdf({
      title: String(draft.title ?? "Response"),
      recipientName: draft.recipient_name as string | null,
      recipientCompany: draft.recipient_company as string | null,
      recipientAddress1: draft.recipient_address1 as string | null,
      recipientAddress2: draft.recipient_address2 as string | null,
      recipientCity: draft.recipient_city as string | null,
      recipientState: draft.recipient_state as string | null,
      recipientPostalCode: draft.recipient_postal_code as string | null,
      subject: draft.subject as string | null,
      body: String(draft.body ?? ""),
      finalizedAt: now,
    });

    const project = await env.DB.prepare(`SELECT cp.project_id FROM case_projects cp JOIN projects p ON p.id = cp.project_id WHERE cp.case_id = ? AND cp.organization_id = ? ORDER BY CASE WHEN cp.role = 'primary' THEN 0 ELSE 1 END LIMIT 1`)
      .bind(caseId, auth.user.organization_id).first<{ project_id: string }>();
    ...
    await env.EVIDENCE_BUCKET.put(r2Key, pdf, {
    ...
    await env.DB.prepare(`INSERT INTO evidence (id, project_id, source, doc_type, title, status, r2_key, case_id, organization_id, response_draft_id, mime_type, file_size, generated_at)
      VALUES (?, ?, 'generated_response', 'response', ?, 'verified', ?, ?, ?, ?, 'application/pdf', ?, ?)`)
      .bind(evidenceId, project.project_id, draft.title, r2Key, caseId, auth.user.organization_id, draftId, pdf.byteLength, now).run();
    ...
    await env.DB.prepare(`INSERT INTO events (id, case_id, event_type, entity_type, entity_id, actor_type, actor_id, actor_name, severity, title, description, payload)
      ...
      .bind(crypto.randomUUID(), caseId, draftId, auth.user.id, auth.user.email ?? "User", "Response finalized", "A response was finalized and captured as a case evidence artifact.", JSON.stringify({ response_draft_id: draftId, evidence_id: evidenceId, r2_key: r2Key })).run();
