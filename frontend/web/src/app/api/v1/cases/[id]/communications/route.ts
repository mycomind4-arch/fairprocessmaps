...
export async function GET(
  ...
  try {
    ...
    ).bind(caseId, auth.user.organization_id).first<{ id: string }>();

    if (!caseRow) return errorResponse("NOT_FOUND", "Case not found", 404);

    const rows = await env.DB.prepare(
      `SELECT id, case_id, purpose, status, mail_class, source_document_id,
              provider, provider_job_id, recipient_name, recipient_company,
              recipient_address1, recipient_address2, recipient_city,
              recipient_state, recipient_postal_code, recipient_country,
              matter_reference, tracking_number, proof_url, error_code,
              error_message, created_at, submitted_at, accepted_at,
...
export async function POST(
  ...
  try {
    ...
    if (!idempotencyKey || idempotencyKey.length > 200) {
      return errorResponse("INVALID_IDEMPOTENCY_KEY", "Idempotency-Key is required and must be 200 characters or fewer", 400);
    }
    ...
    for (const field of required) {
      if (!input[field] || typeof input[field] !== "string") {
        return errorResponse("VALIDATION_ERROR", `${field} is required`, 400);
      }
    }
    ...
    const existing = await db.prepare(
      `SELECT id, status, provider, provider_job_id
        ...
        LIMIT 1`,
    ).bind(auth.user.organization_id, idempotencyKey).first();

    if (existing) {
      return NextResponse.json(
        { ok: true, data: existing, error: null, idempotent_replay: true },
    ...
    await db.batch([
      db.prepare(
        `INSERT INTO case_communications (
          ...
          source_document_id, provider, idempotency_key,
          ...
          matter_reference, metadata, created_at, updated_at
        ) VALUES (?, ?, ?, ?, 'draft', ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).bind(
        communicationId,
        caseId,
        ...
        input.purpose,
        input.mail_class ?? "certified",
        input.source_document_id ?? null,
        idempotencyKey,
        input.recipient_name,
        input.recipient_company ?? null,
        input.recipient_address1,
        input.recipient_address2 ?? null,
        input.recipient_city,
        input.recipient_state,
        input.recipient_postal_code,
        input.recipient_country ?? "US",
        input.matter_reference ?? null,
        metadata,
        now,
        now,
      ),
      db.prepare(
        ...
        auth.user.name,
        "Mail job created",
        "A physical-mail communication was prepared for this case.",
        JSON.stringify({ communication_id: communicationId, purpose: input.purpose }),
      ),
    ]);
    ...
    return NextResponse.json(
      {
        ok: true,
        data: { id: communicationId, case_id: caseId, status: "draft" },
        error: null,
      },
    ...
    if (message.toLowerCase().includes("idx_case_comm_idempotency")) {
      return errorResponse("IDEMPOTENCY_CONFLICT", "The communication already exists; retry the request to retrieve it", 409);
    }
