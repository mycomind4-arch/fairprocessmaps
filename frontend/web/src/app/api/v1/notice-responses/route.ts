...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file instanceof File) { originalFilename = file.name.slice(0, 255); contentType = file.type || "application/octet-stream"; fileHash = await sha256(file); r2Key = `notice-responses/${user.organization_id}/${caseId}/${noticeId}/${fileHash}`; if (env.EVIDENCE_BUCKET) await env.EVIDENCE_BUCKET.put(r2Key, file.stream(), { httpMetadata: { contentType } }); if (!storedNoticeText && (contentType.startsWith("text/") || contentType === "application/json")) storedNoticeText = (await file.text()).slice(0, MAX_TEXT); }
    await db.prepare(`INSERT INTO notice_responses (id, case_id, organization_id, notice_type, agency_name, notice_title, reference_number, received_date, response_deadline, recipient_name, recipient_address1, recipient_address2, recipient_city, recipient_state, recipient_postal_code, notice_text, r2_key, original_filename, content_type, sha256_hash, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(noticeId, caseId, user.organization_id, noticeType, agencyName, noticeTitle, referenceNumber, receivedDate, responseDeadline, recipientName, recipientAddress1, recipientAddress2, recipientCity, recipientState, recipientPostalCode, storedNoticeText, r2Key, originalFilename, contentType, fileHash, user.id, now, now).run();
