    ...
    <div className="min-h-screen bg-fp-bg flex flex-col">
      ...
      <header className="h-14 flex items-center justify-between px-6 glass shrink-0 z-20 border-b border-fp-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-sm tracking-tight text-fp-text leading-none">FairProcess</div>
            <div className="text-[10px] text-fp-text-dim uppercase tracking-wide mt-0.5">Evidence-First Workspace</div>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6" role="main">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-fp-blue text-white text-sm font-medium hover:bg-fp-blue/90 transition-colors"
          >
        ...
        {/* ── Error State ── */}
        {fetchError && (
          <div className="flex items-center justify-between gap-4 p-3 rounded-lg bg-fp-red/10 border border-fp-red/30">
            <div className="flex items-center gap-3">
              <AlertTriangle className="w-4 h-4 text-fp-red shrink-0" />
              <span className="text-sm text-fp-text">{fetchError}</span>
            </div>
            <button
              onClick={loadData}
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-fp-red/20 text-fp-red hover:bg-fp-red/30 transition-all text-xs font-medium"
            >
        ...
        {/* ── Pending Reviews Section (moved above project list) ── */}
        {pendingReviews.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <ClipboardCheck className="w-4 h-4 text-fp-blue" />
              <h2 className="text-sm font-semibold text-fp-text">Pending AI Reviews</h2>
            ...
            <div className="space-y-1.5">
              {pendingReviews.slice(0, 5).map((review) => (
                <div
                  key={review.id}
                  className="flex items-center justify-between gap-4 p-2.5 rounded-lg surface-flat border-fp-blue/20 hover:border-fp-blue/40 transition-all"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-7 h-7 rounded-lg bg-fp-blue/15 flex items-center justify-center shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-fp-text-dim">{review.project_name}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs text-fp-text-dim tabular-nums">{(review.confidence * 100).toFixed(0)}%</span>
        ...
        {/* ── Project List ── */}
        <div className="space-y-2">
          <h2 className="text-sm font-semibold text-fp-text">All Projects</h2>
          {fetching ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="w-12 h-12 rounded-xl surface-flat flex items-center justify-center mb-3">
                <Map className="w-6 h-6 text-fp-text-dim" />
              </div>
              <p className="text-sm text-fp-text-muted">No projects yet</p>
              <p className="text-xs text-fp-text-dim mt-1">Start a new investigation from the map page.</p>
            </div>
          ) : (
            projects.map((p) => (
              <button
                ...
                className="w-full flex items-center gap-4 p-3 rounded-lg surface-flat hover:border-fp-blue/40 text-left transition-all group"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-fp-text group-hover:text-fp-blue transition-colors truncate">{p.name}</span>
                    <span className="text-xs text-fp-text-dim capitalize">{p.case_type.replace(/_/g, " ")}</span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-fp-text-dim mt-1">
                    <span className="font-mono">{p.property.apn}</span>
                    <span className="truncate">{p.property.address || "No address"}</span>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  {p.criticalFindingsCount > 0 && (
                    <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-fp-red/15 text-fp-red border border-fp-red/30">
                      {p.criticalFindingsCount} critical
                    </span>
                  )}
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${p.status === "open" ? "bg-fp-amber/15 text-fp-amber border border-fp-amber/30" : "bg-fp-green/15 text-fp-green border border-fp-green/30"}`}>
                    {p.status}
                  </span>
                  {p.due_process_score != null && (
                    <span className={`text-sm font-semibold tabular-nums ${
                      p.due_process_score >= 80 ? "text-fp-green" : p.due_process_score >= 50 ? "text-fp-amber" : "text-fp-red"
                    }`}>
                      {p.due_process_score}
                    </span>
                  )}
